...
def target_fun(x, a0, a1, a2, a3):
    ...
    return f0
def target_fun_accu(x, a0, t, alpha, a3):
    f0 = (np.abs(a0 * (np.sqrt(1-t**2) - t**2 * np.sqrt(1-alpha)/ (np.exp((x - a3) * 1j) - np.sqrt(1-t**2)* np.sqrt(1-alpha))))) ** 2
    return f0
...
class Smart_Q_fit:
    def __init__(self, root):
        ...
        self.allback = ttk.Button(self.nav_frame, text="from beginning", command=self.Allback, width=10)
        self.allback.pack(side=tk.LEFT, padx=10)
        self.next_button = ttk.Button(self.nav_frame, text="→", command=self.next_step, width=10)
        self.next_button.pack(side=tk.RIGHT, padx=10)

        # 初始化参数
        self.add_peak_slot1 = tk.StringVar(value="0.2")
        self.peakslot1 = tk.StringVar(value="0.1")
...
#%%  Main ui(s)
    ...
    def find_peaks_ui2(self):
        ...
        for i in range(1):
            ...
            ttk.Button(self.FSR_container, text="Hide", command=lambda index=i: self.invert(index),width=5).grid(row=0, column=4, padx=5)
    ...
    def find_peaks_ui(self):
        ...
        for i in range(5):
            ...
            ttk.Button(self.FSR_container, text="Hide", command=lambda index=i: self.invert(index),width=5).grid(row=0, column=4*i+2, padx=5)
            ttk.Button(self.FSR_container, text="Clear", command=lambda index=i: self.destroy_peaks(index),width=5).grid(row=0,
                                                                                                                 column=4*i+3,
                                                                                                                 padx=5)
        self.peaks_container = ttk.Frame(self.content_frame)
    ...
    def analyze_peaks_ui(self):
        ...
        self.couple_var = tk.StringVar(value="Under")
        mode_frame = ttk.Frame(self.content_frame)
        mode_frame.pack(pady=10)
        ttk.Radiobutton(mode_frame, text="Under-Couple", variable=self.couple_var, value="Under",
                        command=self.update_input_fields).grid(row=0, column=0, sticky=tk.W)
        ttk.Radiobutton(mode_frame, text="Critical-Couple", variable=self.couple_var, value="Critical",
                        command=self.update_input_fields).grid(row=1, column=0, sticky=tk.W)
        ttk.Radiobutton(mode_frame, text="Over-Couple", variable=self.couple_var, value="Over",
                        command=self.update_input_fields).grid(row=2, column=0, sticky=tk.W)
...
#%% Extra functions used
    ...
    def export_preparation(self):
        ...
        if self.export == 1:
            ...
            # 创建输出文件名
            self.output_name = os.path.join(self.save_folder_path, f"{file_name}\\categorized_peaks.csv")
            folder_name= os.path.join(self.save_folder_path, f"{file_name}")
    ...
    def fit_Q(self, j):
        ...
        for m in range(1):
            ...
            freqs0=3e8/(self.filtered_x[peaks]*1e-9)
            orders=self.orders[j]
            median = np.percentile(orders, 50, interpolation='nearest')
            median_index = np.where(orders == median)[0][0]
            orders=orders-median
            freqs=freqs0-freqs0[median_index]
            THRE=40
            params = np.polyfit(orders[(orders>-THRE)&(orders<THRE)], freqs[(orders>-THRE)&(orders<THRE)], 2)
            a = params[0]*2
            b = params[1]
            freqs=freqs-orders*b
            plot_frame = tk.Frame(self.scrollable_frame)
            plot_frame.pack(side=tk.TOP, fill=tk.BOTH, expand=True, pady=10)
            # 创建一个 Matplotlib 图表
            fig, ax = plt.subplots(figsize=(16, 6))
            ax.scatter(orders, freqs, label='Data')
            ax.plot(orders, a*orders**2/2, label='Fit')
            title=fr"$D_2$ fit"
            ax.set_title(title)
            ax.set_xlim(orders.min(), 1.4 * orders.max() - 0.4*orders.min())
            x_annotation = (max(orders) - min(orders)) * (-0.1) + max(orders)  # 注释的 x 位置为数据最大值的 1.1 倍，确保在图表右侧
            y_annotation = (max(freqs)+min(freqs)) * 0.5  # 注释的 y 位置为数据最大值的 0.5 倍，确保在图表中心
            annotation_text=fr"$D_2={a/1e9}GHz$"
            ...
            # 为每个 FSR 创建一个文件夹
            folder_name = os.path.join(self.save_folder_path, f"{self.file_name0.get()}\\FSR={round(self.fsrs[j],4)}nm")
            if not os.path.exists(folder_name):
                        os.makedirs(folder_name)
            self.image_name = 'D_2'
            ...
            for i in peaks:
                ...
                p0 = [.1, 0.5, 0.5, self.filtered_x[i]*2*np.pi/self.fsrs[j]]
                popt, pcov = optimize.curve_fit(target_fun_accu, resonance_1_x*2*np.pi/self.fsrs[j], resonance_1_y,
                                                p0=p0, bounds=(0, np.inf), maxfev=5000)
                y_fit = np.array([target_fun_accu(ii, *popt) for ii in resonance_1_x*2*np.pi/self.fsrs[j]])
                r2 = r2_score(resonance_1_y, target_fun_accu(resonance_1_x*2*np.pi/self.fsrs[j], *popt))
                if r2>0.2:
                    t2 = popt[1]**2
                    alpha = popt[2]
                    wl_center = popt[3]*self.fsrs[j]/(2*np.pi)
                    lw_e = t2*self.fsrs[j]/(2*np.pi)
                    lw_i = alpha*self.fsrs[j]/(2*np.pi)
                    print(lw_e,lw_i)
                    lw = lw_e + lw_i
                    Q = wl_center / lw
                    Q_e = wl_center / lw_e
                    Q_i = wl_center / lw_i
                    if self.couple_var.get()=="Under":
                        Qs.append(min(Q_e,Q_i))
                        Qes.append(max(Q_e,Q_i))
                        Qi=Qs[-1]
                        Qe=Qes[-1]
                    elif self.couple_var.get()=="Critical":
                        Qs.append((Q_e+Q_i)/2)
                        Qes.append((Q_e+Q_i)/2)
                        Qi=Qs[-1]
                        Qe=Qes[-1]
                    else:
                        Qs.append(max(Q_e,Q_i))
                        Qes.append(min(Q_e,Q_i))
                        Qi=Qs[-1]
                        Qe=Qes[-1]
                    lw_e=wl_center/Qe
                    lw_i=wl_center/Qi
                    ...
                    wls.append(round(lw * 10 ** 3, 2))
                    # 生成图像
                    self.create_plot(j, resonance_1_x, resonance_1_y, y_fit,
                                    f"FSR={round(self.fsrs[j], 3)}nm, wl={self.filtered_x[i]}nm", to_write, i, r2)
        ...
        print(data)
        # 自动生成 10 个等宽区间
        bins = np.arange(0, max(round(np.max(data)+0.1,1),2), 0.1)
        print(bins)  # 10个小条需要11个边界点
        ax.hist(data, bins=bins, edgecolor='black')
        ax.set_xlabel('Million')
        ax.set_ylabel('Counts')
        title=fr"$Q_i$ Distribution"
        ax.set_title(title)
            # 将图表嵌入到 Tkinter 窗口中
        canvas = FigureCanvasTkAgg(fig, master=plot_frame)
        canvas.draw()
        ...
        plot_frame.pack(side=tk.TOP, fill=tk.BOTH, expand=True, pady=10)
            # 创建一个 Matplotlib 图表
        fig, ax = plt.subplots(figsize=(16, 6))
        ax.scatter(wls, np.log10(Qes/1e6))
        ax.plot(wls, np.log10(Qes/1e6)*0)
        title=fr"$Q_e$"
        ax.set_title(title)
            # 将图表嵌入到 Tkinter 窗口中
        canvas = FigureCanvasTkAgg(fig, master=plot_frame)
        ...
        if self.exports[j] ==1:
            if not os.path.exists(folder_name):
                        os.makedirs(folder_name)
                    # 保存图像
            self.image_name = f'Q_e.svg'
            self.new_image_path = os.path.join(folder_name, self.image_name)
            plt.savefig(self.new_image_path, dpi=700, bbox_inches='tight') 
        
    def create_plot(self, plot_number, data_x, data_y, fit_y, title, annotation_text, peak_n, r):
        # 创建一个子框架
        plot_frame = tk.Frame(self.scrollable_frame)
        plot_frame.pack(side=tk.TOP, fill=tk.BOTH, expand=True, pady=10)
        ...
        y_annotation = (max(data_y)) * 0.5  # 注释的 y 位置为数据最大值的 0.5 倍，确保在图表中心
        ax.annotate(annotation_text, xy=(x_annotation, y_annotation), xytext=(x_annotation, y_annotation),
                    arrowprops=dict(facecolor='black', shrink=0.05), ha='left', va='center')
        ax.legend()
        # 将图表嵌入到 Tkinter 窗口中
        ...
        if self.exports[plot_number] == 1:
            ...
            # 为每个 FSR 创建一个文件夹
            folder_name = os.path.join(self.save_folder_path, f"{self.file_name0.get()}\\FSR={round(self.fsrs[plot_number],4)}nm")

            # 保存图像
            self.image_name = f'{self.filtered_x[peak_n]}nm, r2={r}.png'
            self.new_image_path = os.path.join(folder_name, self.image_name)
            plt.savefig(self.new_image_path, dpi=700, bbox_inches='tight') 
...
# 设置新的 y 轴范围，只更新下限
        new_ylim = (0, current_ylim[1])  # 0 是新的下限，current_ylim[1] 是当前的上限
    ...
    def add_peak(self):
        ...
        self.cid=self.fig.canvas.mpl_connect('button_press_event', self.on_click_add_peak)
    ...
    def on_click_add_peak(self, event):
        ...
        if self.peak_click_stage == 1:
            ...
            self.peak_in_y = self.filtered_y[(self.filtered_x > event.xdata - self.add_peak_slot / 2) & (
                    self.filtered_x < event.xdata + self.add_peak_slot / 2)]
            peaks1, _ = find_peaks(-self.peak_in_y, height=-(event.y + np.max(self.filtered_y)*0.01), threshold=0, distance=500, prominence=.005,
                                   width=None,
                                   wlen=None,  plateau_size=None)
            ...
            self.peak_in_y = self.filtered_y[
                (self.filtered_x > event.xdata - self.peakslot / 2) & (
                            self.filtered_x < event.xdata + self.peakslot / 2)]
            peaks1, _ = find_peaks(-self.peak_in_y, height=-(event.y + np.max(self.filtered_y)*0.01), threshold=0, distance=500,
    ...
    def on_click_add_single_peak(self, event, i):
        ...
        self.peak_in_y = self.filtered_y[(self.filtered_x > event.xdata - self.add_peak_slot / 2) & (
                    self.filtered_x < event.xdata + self.add_peak_slot / 2)]
        peaks1, _ = find_peaks(-self.peak_in_y, height=-(event.y + 0.05*np.max(self.filtered_y)), threshold=0, distance=500, prominence=.005,
                                   width=None,
                                   wlen=None, rel_height=0.5, plateau_size=None)
        index = self.peak_in_y[peaks1].argmin()
        peak_index = peaks1[index] + self.filtered_x.tolist().index(self.peak_in_x[0])
    ...
    def extend(self, i):
        if len(self.peaks[i]) > 1:
            approximate_fsr =(self.peaks[i][-1]-self.peaks[i][-2])/[self.orders[i][-1]-self.orders[i][-2]]
            ...
            while x0 + peakslot / 2 < self.filtered_x[-1]:
                ...
                if np.max(tryslot_y) - np.min(tryslot_y) > 0.01:
                    ...
                    try_limit = -1
                peakstry, _ = find_peaks(-tryslot_y, height=-try_limit-0.1*np.max(self.filtered_y), threshold=0, distance=500, prominence=.005,
                                         width=None, wlen=None, plateau_size=None)
                if peakstry.shape[0]==0:
                    approximate_fsr =(self.peaks[i][-1]-self.peaks[i][-2])/[self.orders[i][-1]-self.orders[i][-2]]
                    x0 += approximate_fsr
                    order += 1
                else:
                    index = tryslot_y[peakstry].argmin()
                    formerx0 = float(x0 - approximate_fsr)
                    x0_index = peakstry[index] + try_index[0][0]
                    ...
                    self.fsrs[i] = m
                    approximate_fsr =(self.peaks[i][-1]-self.peaks[i][-2])/[self.orders[i][-1]-self.orders[i][-2]]
            approximate_fsr =(self.peaks[i][1]-self.peaks[i][0])/[self.orders[i][1]-self.orders[i][0]]
            ...
            while x0 - peakslot / 2 > self.filtered_x[0]:
                ...
                if peakstry.shape[0]==0:
                    ...
                    x0_index = peakstry[index] + try_index[0][0]
                    x0 = self.filtered_x[x0_index]
                    self.peaks[i].insert(0,x0)
                    self.powers[i].insert(0,self.filtered_y[x0_index])
                    self.peaks_index[i].insert(0,x0_index)
                    approximate_fsr =(self.peaks[i][1]-self.peaks[i][0])/[self.orders[i][1]-self.orders[i][0]]
                    certain_peaks.insert(0,x0_index)
                    order -= 1
                    self.orders[i].insert(0,order)
                    x0 -= approximate_fsr
                    coeffs = np.polyfit(self.orders[i], self.peaks[i], 1)
                    # coeffs[0] 是斜率 m，coeffs[1] 是截距 b
                    m, b = coeffs
    ...
    def invert(self,i):
        self.plot[i] = not self.plot[i]
        self.plot_peaks()

    def renew_for_one(self, i):
        self.renew_para()
        self.moredel = 1
        self.cid=self.fig.canvas.mpl_connect('button_press_event', lambda event, index=i: self.delete_certain_peak(event, index))
