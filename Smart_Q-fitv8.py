...
class Smart_Q_fit:
    ...
    def fit_Q(self, j):
        ...
        plot_frame.pack(side=tk.TOP, fill=tk.BOTH, expand=True, pady=10)
            # 创建一个 Matplotlib 图表
        fig, ax = plt.subplots(figsize=(16, 6))
        ax.plot(freqs0, np.log10(Qes/1e6))
        ax.plot(freqs0, np.log10(Qes/1e6)*0)
        title=fr"$Q_e"
        ax.set_title(title)
            # 将图表嵌入到 Tkinter 窗口中
        canvas = FigureCanvasTkAgg(fig, master=plot_frame)
    ...
    def create_plot(self, plot_number, data_x, data_y, fit_y, title, annotation_text, peak_n, r):
        ...
        if self.exports[plot_number] == 1:
            ...
            # 为每个 FSR 创建一个文件夹
            folder_name = os.path.join(self.save_folder_path, f"{self.file_name0.get()}_FSR={self.fsrs[plot_number]}nm")
            if not os.path.exists(folder_name):
                os.makedirs(folder_name)
    ...
    def on_click_add_peak(self, event):
        ...
        if self.peak_click_stage == 1:
            ...
            self.peak_in_y = self.filtered_y[(self.filtered_x > event.xdata - self.add_peak_slot / 2) & (
                    self.filtered_x < event.xdata + self.add_peak_slot / 2)]
            peaks1, _ = find_peaks(-self.peak_in_y, height=-(event.y + np.max(self.filtered_y)*0.1), threshold=0, distance=500, prominence=.005,
                                   width=None,
                                   wlen=None, rel_height=0.5, plateau_size=None)
            ...
            self.peak_in_y = self.filtered_y[
                (self.filtered_x > event.xdata - self.peakslot / 2) & (
                            self.filtered_x < event.xdata + self.peakslot / 2)]
            peaks1, _ = find_peaks(-self.peak_in_y, height=-(event.y + np.max(self.filtered_y)*0.1), threshold=0, distance=500,
        ...
        if len(self.peaks[i]) > 1:
            ...
            while x0 + peakslot / 2 < self.filtered_x[-1]:
                ...
                peakstry, _ = find_peaks(-tryslot_y, height=-try_limit, threshold=0, distance=500, prominence=.005,
                                         width=None, wlen=None, rel_height=0.5*np.max(self.filtered_y), plateau_size=None)
                if not peakstry:
                    x0 += approximate_fsr
                    order += 1
                else:
